...
resource "aws_security_group" "main" {
  name        = "vpce-sg-main"
  description = "VPC endpoint SG main"
  vpc_id      = aws_vpc.main.id
}

resource "aws_security_group" "extra" {
  name        = "vpce-sg-extra"
  description = "VPC endpoint SG extra"
  vpc_id      = aws_vpc.main.id
}
...
resource "aws_vpc_endpoint_security_group_association" "extra" {
  vpc_endpoint_id = aws_vpc_endpoint.iface.id
  security_group_id = aws_security_group.extra.id
}
